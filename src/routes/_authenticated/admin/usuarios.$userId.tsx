...
const card = "rounded-xl border border-slate-800 bg-slate-900/50 p-4";
const th = "text-left font-medium px-3 py-2 text-slate-400";
...
function AdminUserDetail() {
  ...
  return (
    <div className="space-y-4">
      <Link to="/admin" className="inline-flex items-center gap-2 text-sm text-slate-400 hover:text-slate-200">
        <ArrowLeft className="h-4 w-4" /> Voltar para usuários
      </Link>

      {q.isLoading && <div className="text-sm text-slate-400">Carregando dados do usuário...</div>}
        ...
        <>
          <div className="flex flex-wrap items-center gap-3">
            <div>
              ...
              <div className="text-sm text-slate-400">{d.auth.email}</div>
            </div>
            ...
            <TabsContent value="perfil" className="grid gap-3 md:grid-cols-2">
              <div className={card}>
                ...
                <dl className="grid grid-cols-2 gap-2 text-sm">
                  <dt className="text-slate-400">Peso</dt><dd>{d.profile?.peso ?? "—"} kg</dd>
                  <dt className="text-slate-400">Peso meta</dt><dd>{d.profile?.peso_meta ?? "—"} kg</dd>
                  <dt className="text-slate-400">Altura</dt><dd>{d.profile?.altura ?? "—"} m</dd>
                  <dt className="text-slate-400">Idade</dt><dd>{d.profile?.idade ?? "—"}</dd>
                  <dt className="text-slate-400">Objetivo</dt><dd>{d.profile?.objetivo ?? "—"}</dd>
              ...
              <div className={card}>
                ...
                <dl className="grid grid-cols-2 gap-2 text-sm">
                  <dt className="text-slate-400">Calorias</dt><dd>{d.goals?.calorias ?? "—"} kcal</dd>
                  <dt className="text-slate-400">Proteínas</dt><dd>{d.goals?.proteinas ?? "—"} g</dd>
                  <dt className="text-slate-400">Carboidratos</dt><dd>{d.goals?.carboidratos ?? "—"} g</dd>
                  <dt className="text-slate-400">Gorduras</dt><dd>{d.goals?.gorduras ?? "—"} g</dd>
                  <dt className="text-slate-400">Fibras</dt><dd>{d.goals?.fibras ?? "—"} g</dd>
                  <dt className="text-slate-400">Meta de água</dt><dd>{d.waterGoal ?? "—"} ml</dd>
                </dl>
              </div>
            </TabsContent>
                    ...
                    <tbody>
                      {d.meals.map((m) => (
                        <tr key={m.id} className="border-t border-slate-800 align-top">
                          <td className="px-3 py-2 text-slate-400">{new Date(m.data + "T00:00:00").toLocaleDateString("pt-BR")}</td>
              ...
              <div className={card}>
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {d.workouts.map((w) => (
                        <tr key={w.id} className="border-t border-slate-800 align-top">
                          <td className="px-3 py-2 text-slate-400">{new Date(w.data + "T00:00:00").toLocaleDateString("pt-BR")}</td>
                  ...
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {waterByDay.map(([dia, ml]) => (
                        <tr key={dia} className="border-t border-slate-800">
                          <td className="px-3 py-2 text-slate-400">{new Date(dia + "T00:00:00").toLocaleDateString("pt-BR")}</td>
            ...
            <TabsContent value="evolucao">
              <div className={card}>
                ...
                <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 md:grid-cols-4">
                  {d.photos.map((p) => (
                    <figure key={p.id} className="overflow-hidden rounded-lg border border-slate-800">
                      {p.url ? (
                        <img
                          src={p.url}
